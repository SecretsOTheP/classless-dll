...
{ 
    ...
    { 
		...
		case 508:
			...
			break;
		case 509:
			sprintf(szTemp, "Pet Uses Skill on Caster's Target: %s", GetSkillNameFromID(pSpell->Base[i]));
			strcat(szBuff, szTemp);
			break;
		case 510:
			sprintf(szTemp, "Pet Uses Spell on Caster's Target: %s", GetSpellByID(pSpell->Base[i]) ? GetSpellByID(pSpell->Base[i])->Name : "NONE");
			strcat(szBuff, szTemp);
			break;
		case 511:
...
char* GetSkillNameFromID(DWORD skillID)
{

	if (skillID > 0)
	{
		if (PSKILL pSkill = pSkillMgr->pSkill[skillID])
			return pStringTable->getString(pSkill->nName,0);
	}
